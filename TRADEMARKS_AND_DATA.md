...
本 repository 不包含正式資料庫備份，也不附帶政府資料開放平臺的全量目錄快照；治理來源卡與同步程式只協助定位及判讀來源，不保證每筆上游資料都可公開重製、建立衍生著作或商業使用。
...
本專案程式碼與技術文件的著作權人以 `LICENSE` 檔所載為準。品牌識別的權利人為 PiSuAI 之經營主體。兩者可能不同；任何權利授權或爭議，請依 `SECURITY.md` 或 repository 所列聯絡方式洽詢。
