...
import { normalizeGovernmentRow, previousTaipeiDate } from "./governmentCatalog";
...
describe("government catalog normalization", () => {
  it("maps official Chinese metadata fields without copying contact details", () => {
    ...
    expect(row).toMatchObject({ datasetId: "6564", title: "政府資料集清單", status: "active", lastSeenAt: 123456 });
  ...
  it("preserves withdrawn state from a delta record", () => {
    expect(normalizeGovernmentRow({
      資料集識別碼: "88",
      資料集名稱: "已下架資料",
      服務分類: "公共資訊",
      提供機關: "測試機關",
      資料集變動狀態: "資料集下架",
    }, 10)?.status).toBe("withdrawn");
  });

  it("uses the previous calendar date in Taipei", () => {
    expect(previousTaipeiDate(new Date("2026-09-15T00:30:00Z"))).toBe("2026-09-14");
  });
});
