...
export const NATIONAL_CATALOG_URL = "https://data.gov.tw/api/v2/rest/dataset/export";
export const NATIONAL_DELTA_URL = "https://data.gov.tw/api/front/dataset/changed/export";
...
function parseTaipeiDate(value: unknown) {
  const raw = typeof value === "object" && value && "date" in value
    ...
    : text(value);
  if (!raw) return null;
  const timestamp = Date.parse(raw.includes("T") ? raw : `${raw.replace(" ", "T")}+08:00`);
  return Number.isFinite(timestamp) ? timestamp : null;
}

export function normalizeGovernmentRow(row: GovernmentRow, observedAt: number): NormalizedDataset | null {
  ...
  const status = statusText?.includes("下架") ? "withdrawn" as const : "active" as const;
  const rawHash = createHash("sha256").update(JSON.stringify(row)).digest("hex");
  ...
  return {
    ...
    publicationMethod: text(row["資資料集上架方式"] ?? row["資料集上架方式"], 120),
...
async function upsertBatch(rows: NormalizedDataset[]) {
  ...
  const existing = await db
    .select({ datasetId: governmentDatasets.datasetId, rawHash: governmentDatasets.rawHash, status: governmentDatasets.status })
    .from(governmentDatasets)
    .where(inArray(governmentDatasets.datasetId, ids));
  const old = new Map(existing.map(row => [row.datasetId, `${row.rawHash}:${row.status}`]));
  const changed = rows.filter(row => old.get(row.datasetId) !== `${row.rawHash}:${row.status}`).length;

  await db.insert(governmentDatasets).values(rows).onDuplicateKeyUpdate({
    set: {
...
async function startSync(mode: "full" | "delta", reportDate?: string) {
  const db = await requireDb();
  const [created] = await db.insert(governmentCatalogSyncs).values({
    mode,
    status: "running",
...
async function finishSync(id: number, values: Partial<typeof governmentCatalogSyncs.$inferInsert>) {
  const db = await requireDb();
  await db.update(governmentCatalogSyncs).set({ ...values, finishedAt: Date.now() }).where(eq(governmentCatalogSyncs.id, id));
}

export async function syncGovernmentCatalogFile(path: string) {
  const syncStartedAt = Date.now();
  const syncId = await startSync("full");
  ...
  try {
    ...
    for await (const raw of parser) {
      ...
      if (seen % 5000 === 0) console.log(`[GovernmentCatalog] imported ${seen} datasets`);
    }
    changed += await upsertBatch(batch);
    const db = await requireDb();
    await db.update(governmentDatasets).set({ status: "withdrawn", updatedAt: Date.now() }).where(and(lt(governmentDatasets.lastSeenAt, syncStartedAt), eq(governmentDatasets.status, "active")));
    const snapshotSha256 = hasher.digest("hex");
    await finishSync(syncId, { status: "succeeded", recordsSeen: seen, recordsChanged: changed, snapshotSha256, message: `Full catalog synchronized: ${seen} datasets.` });
    return { ok: true, mode: "full" as const, recordsSeen: seen, recordsChanged: changed, snapshotSha256 };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    await finishSync(syncId, { status: "failed", recordsSeen: seen, recordsChanged: changed, message });
    throw error;
  }
}
...
export async function syncGovernmentCatalogDelta(reportDate: string) {
  ...
  try {
    ...
    const response = await fetch(url, { headers: { "User-Agent": "PiSuAI-OpenData-Indexer/1.0" }, signal: AbortSignal.timeout(90_000) });
    if (!response.ok) throw new Error(`data.gov.tw delta returned HTTP ${response.status}`);
    const body = await response.text();
    const rows = JSON.parse(body) as GovernmentRow[];
    let batch: NormalizedDataset[] = [];
    for (const raw of rows) {
      const changeStatus = text(raw["資料集變動狀態"]);
      if (!changeStatus || !["資料集上架", "資料集修改", "資料集下架"].includes(changeStatus)) continue;
    ...
    await finishSync(syncId, { status: "succeeded", recordsSeen: seen, recordsChanged: changed, snapshotSha256, message: `Delta ${reportDate}: ${seen} datasets.` });
    return { ok: true, mode: "delta" as const, reportDate, recordsSeen: seen, recordsChanged: changed, snapshotSha256 };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    await finishSync(syncId, { status: "failed", recordsSeen: seen, recordsChanged: changed, message });
    throw error;
