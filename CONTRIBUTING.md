...
完整政府目錄快照、大型生成索引、未經核准的資料庫 dump、備份檔或匯出檔不得提交至 Git 歷史。完整快照只能以 GitHub Release 資產提供，完整 `catalog/` 只能由 Actions 建置。repository 只接受生成腳本、manifest、NOTICE 及 `data/catalog-sample.json` 的 100 筆最小樣本；任何超過 1 MB 的新追蹤檔都會被 CI 拒絕，除非維護者先以獨立提案核准並同步調整 CI 政策。
