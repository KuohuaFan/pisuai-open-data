...
INNER JOIN `automation_configs` AS `legacy`
 ...
 AND `legacy`.`key` = 'generate-biennial-report'
...
INNER JOIN `automation_configs` AS `current`
  ON `current`.`key` = 'generate-report-draft'
WHERE `legacy`.`key` = 'generate-biennial-report';
--> statement-breakpoint
-- If only the legacy key exists, rename it in place so the task UID and history
-- stay attached to the same row. Re-running this statement changes nothing.
UPDATE `automation_configs`
SET `key` = 'generate-report-draft',
    `updatedAt` = CAST(UNIX_TIMESTAMP(CURRENT_TIMESTAMP(3)) * 1000 AS UNSIGNED)
WHERE `key` = 'generate-biennial-report'
  AND NOT EXISTS (
    SELECT 1
    FROM (
      SELECT `id`
...
-- UPDATE `automation_configs`
-- SET `key` = 'generate-biennial-report',
--     `updatedAt` = CAST(UNIX_TIMESTAMP(CURRENT_TIMESTAMP(3)) * 1000 AS UNSIGNED)
-- WHERE `key` = 'generate-report-draft'
--   AND NOT EXISTS (
--     SELECT 1
