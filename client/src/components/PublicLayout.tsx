  ...
  return (
    ...
    >
      <img
        src={BRAND.logoPath}
        alt=""
        className="size-10 rounded-full object-cover shadow-sm transition-transform duration-200 group-hover:-translate-y-0.5"
      />
      {!compact && (
        <span className="leading-none">
          <span className="block font-serif text-lg font-semibold tracking-wide text-ink">
            {BRAND.shortName}
          </span>
          <span className="mt-1 block text-[10px] font-semibold tracking-[0.28em] text-muted-foreground">
            {BRAND.subtitle}
          </span>
        </span>
      )}
        ...
        {open && (
          <div className="border-t border-ink/10 bg-paper px-5 py-5 md:hidden">
            <nav className="flex flex-col gap-4">
              ...
              {!user ? (
                <Button onClick={() => startLogin()}>登入收藏</Button>
              ) : (
        ...
        <div className="container grid gap-10 py-12 md:grid-cols-[1.3fr_1fr_1fr]">
          <div>
            <div className="mb-4 inline-flex items-center gap-3">
              <BrandMark compact />
              <strong className="font-serif text-lg">{BRAND.name}</strong>
            </div>
            <p className="max-w-md text-sm leading-7 text-white/62">
              來源可追、版本可核、權利先行、錯誤可改。公開資料不是終點，而是可被驗證的知識起點。
            </p>
          </div>
          <div>
            <p className="footer-title">探索</p>
            <div className="space-y-3 text-sm text-white/68">
              <Link href="/sources">治理來源庫</Link>
              <br />
              <Link href="/government-data">政府資料全集</Link>
              <br />
              <Link href="/reports">深度報導</Link>
              <br />
              <Link href="/methodology">治理方法</Link>
            </div>
          </div>
          <div>
            <p className="footer-title">治理承諾</p>
            <p className="flex items-start gap-2 text-sm leading-6 text-white/68">
              <ShieldCheck className="mt-0.5 size-4 shrink-0 text-jade" />
              所有公開內容均保留來源、權利分級與更正狀態；AI
              草稿不得跳過人工發布。
            </p>
          </div>
        </div>
